...
export class PhotographerAvailabilityController {
  ...
  @ApiOperation({
    summary:
      'Crear o actualizar disponibilidad de un fotógrafo autenticado para un día',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos o franjas inexistentes',
  })
  ...
  @Get()
  @ApiOperation({
    summary: 'Obtener disponibilidad semanal del fotógrafo autenticado',
  })
  @ApiResponse({
    status: 200,
  ...
  async findAll(@Req() req: Request) {
    const photographerId = req.user?.userId;
    return this.photographerAvailabilityService.findAllForPhotographer(
      photographerId,
    );
  }
}
